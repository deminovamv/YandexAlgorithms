...
def main():
    elections_d = {}
    with open("input_2b.txt", "r", encoding="utf-8") as f:
        for line in f:
            name, number = line.split()
            if name in elections_d:
                elections_d[name] += int(number)
            else:
                elections_d[name] = int(number)
    list_elections = list(elections_d.keys())
    list_elections.sort()
    for i in list_elections:
        print(i, elections_d[i])
